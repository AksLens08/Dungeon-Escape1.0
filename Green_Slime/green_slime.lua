...
GreenSlime.HITBOX_W = 22
GreenSlime.HITBOX_H = 18
...
function GreenSlime:init(x, y)
    self.w, self.h = GreenSlime.HITBOX_W, GreenSlime.HITBOX_H
...
function GreenSlime:updateTexture()
    local texKey = "green_slime_" .. self.state
    self.texture = gTextures[texKey] or gTextures["green_slime_idle"]
...
function GreenSlime:update(dt, player, dungeon)
...
function GreenSlime:updateAI(dt, player, dungeon)
    local px, py = player:getCenter()
    local sx, sy = self:getCenter()
...
function GreenSlime:handleAnimation(dt)
...
function GreenSlime:takeDamage(amount, attacker, dungeon, kbMult)
