...
function Knight:init(x, y)
    ...
    self.armorRegenTimer = 0
    self.armorRegenInterval = 2.5
...
function Knight:update(dt, map, gMouse)
    ...
    if self.hp <= 0 then
        ...
        if self.armor < 100 then
            self.armorRegenTimer = self.armorRegenTimer + dt
            if self.armorRegenTimer >= self.armorRegenInterval then
                self.armor = math.min(100, self.armor + 10) -- Regenerate 10 armor
                self.armorRegenTimer = 0
            end
        end
        ...
        elseif (gMouse and gMouse.leftDown) and self.attackCooldown <= 0 then
            ...
            self.attackCooldown = 0.7 -- Increased to allow enemies a window to react
