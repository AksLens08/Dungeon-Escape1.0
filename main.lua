...
local Enemy    = require("wizard.wizard")
...
gameState = "menu"
local mainMenu, player, dungeon, lighting, coins, isPaused, camera, enemies, jumpScareSounds, projectiles, minimap
local jumpScareTimer, jumpScareDuration = 0, 2
local kmEscapeAvailable, kmEscapeUsed, caughtKM = false, false, nil
...
local function playFireballSound()
    if fireballSound then
        fireballSound:clone():play()
    end
end
...
local function startGame()
    ...
    enemies = {}
    kmEscapeAvailable = false
    kmEscapeUsed = false
    caughtKM = nil

    local kmSpawns = dungeon:getSpawnPointsOutsideSafeRoom(1, KM.HITBOX_W, KM.HITBOX_H, SPAWN_PADDING)
    if #kmSpawns > 0 then
        table.insert(enemies, KM:new(kmSpawns[1].x, kmSpawns[1].y))
    else
        print("Warning: Skipped KM spawn because no walkable area could fit it.")
    end
    ...
    while #coins < COINS_REQUIRED and attemptedCoins < COINS_REQUIRED * 10 do
        ...
        if x and y then
            ...
            if not duplicate then
                table.insert(coins, Coin:new(x, y))
            end
        end
    end
...
function love.load()
    ...
    Coin.load()
    fireballSound = love.audio.newSource("Audios/fireball.mp3", "static")
            ...
            for i = #enemies, 1, -1 do
                ...
                if enemy.caught and gameState == "play" then
                    ...
                    if enemy.name == "KM" then
                        caughtKM = enemy
                        kmEscapeAvailable = not kmEscapeUsed
                    end
                ...
                if player.state == "attack" and player.frame == 2 and enemy.hp > 0 then
                    ...
                    if distSq < 1000 then
                        -- The wizard now has a 15% chance to receive knockback on hit.
...
function love.keypressed(key)
    if key == "escape" then
        love.event.quit()
    elseif key == "space" and (gameState == "dead" or gameState == "gameover") and kmEscapeAvailable and caughtKM then
        gameState = "play"
        jumpScareTimer = 0
        kmEscapeUsed = true
        kmEscapeAvailable = false
        if caughtKM and caughtKM.stun then
            caughtKM.caught = false
            caughtKM:stun(2)
        end
        caughtKM = nil
        if player then
...
function love.draw()
    ...
    if gameState == "dead" then
        ...
        if kmEscapeAvailable then
            love.graphics.setFont(gFonts["hud"])
            love.graphics.printf("Press SPACE to escape!", 0, sh * 0.35, sw, "center")
        end
